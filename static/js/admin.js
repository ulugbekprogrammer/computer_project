...
async function fetchRepairs(filter = 'all') {
    ...
    try {
        ...
        repairs = repairs.filter(repair => {
            if (filter === 'pending') return !repair.completed;
        ...
        repairs.forEach(repair => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="table-actions">
                        <button class="action-btn action-btn-toggle" onclick="toggleRepairCompletion(${repair.id}, ${!repair.completed})">
                            <i class="fas fa-${repair.completed ? 'undo' : 'check'}"></i>
                        </button>
...
// Toggle repair completion
async function toggleRepairCompletion(repairId, completed) {
    showLoading();
    try {
        const response = await fetch(`${API_BASE_URL}/admin/repairs/${repairId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to update completion status: ${errorData.detail || response.statusText}`);
        }

        repairFilter.value = 'all';
        await fetchRepairs('all');
        showToast('success', 'Success', `Repair marked as ${completed ? 'completed' : 'pending'}`);
        hideLoading();
    } catch (error) {
        hideLoading();
...
// Placeholder edit repair
window.editRepair = function(id) {
    showToast('warning', 'Warning', 'Repair editing not implemented');
};
...
window.deleteRepair = async function(id) {
    ...
    showLoading();
    try {
        const response = await fetch(`${API_BASE_URL}/admin/repairs/${id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
        ...
        await fetchRepairs(repairFilter.value);
        showToast('success', 'Success', 'Repair deleted successfully');
        hideLoading();
    } catch (error) {
        hideLoading();
        showToast('error', 'Error', 'Failed to delete repair');
